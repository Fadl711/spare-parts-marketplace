        ...
        await register(
          {
            ...
            phone,
            city: city || "غير محدد", // Provide default if empty
          },
          "customer"
        );
      } else {
        ...
        <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
          ...
          <View className="px-6 pt-8 pb-6">
            ...
            <Text className="text-gray-500 text-right mb-6">
              {isLogin ? "سجل دخولك للمتابعة" : "أنشئ حساباً للبدء"}
            </Text>
            ...
            <View className="flex-row bg-gray-100 p-1 rounded-xl mb-6">
              <TouchableOpacity
                className={`flex-1 py-3 rounded-lg ${
                  userType === "seller" ? "bg-white shadow-sm" : ""
                }`}
                onPress={() => setUserType("seller")}
              >
                <Text
                  className={`text-center font-bold ${
                    userType === "seller" ? "text-purple-600" : "text-gray-500"
                  }`}
                >
                  بائع 🏪
                </Text>
              ...
              <TouchableOpacity
                className={`flex-1 py-3 rounded-lg ${
                  userType === "customer" ? "bg-white shadow-sm" : ""
                }`}
                onPress={() => setUserType("customer")}
              >
                <Text
                  className={`text-center font-bold ${
                    userType === "customer" ? "text-blue-600" : "text-gray-500"
                  }`}
                >
                  مشتري 🛒
                </Text>
          ...
          <View className="px-6">
            {!isLogin && (
              <>
                ...
                <View className="mb-4">
                  <Text className="text-gray-700 text-right mb-2 font-medium">
                    {userType === "seller" ? "اسم المالك" : "الاسم الكامل"}
                  </Text>
                  <TextInput
                    className="bg-gray-50 px-4 py-3 rounded-xl text-right"
            ...
            >
              {loading ? (
                ...
                <Text className="text-white font-bold text-lg">
                  {isLogin
                    ? `تسجيل دخول ${userType === "customer" ? "مشتري" : "بائع"}`
                    : "إنشاء الحساب"}
                </Text>
              )}
            </TouchableOpacity>
            ...
            <View className="flex-row items-center justify-center mt-6 mb-8">
              <TouchableOpacity onPress={() => setIsLogin(!isLogin)}>
                <Text
                  className={`font-bold ${
                    userType === "seller" ? "text-purple-600" : "text-blue-600"
                  }`}
                >
