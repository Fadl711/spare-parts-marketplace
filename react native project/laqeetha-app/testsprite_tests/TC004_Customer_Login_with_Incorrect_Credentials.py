...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'تسجيل دخول مشتري' (login button) again to confirm login failure behavior and then extract any new messages or page changes.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=مرحباً بعودتك 👋').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=سجل دخولك للمتابعة').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=بائع 🏪').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=مشتري 🛒').first).to_be_visible(timeout=30000)
        ...
        await expect(frame.locator('text=تسجيل دخول مشتري').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=إنشاء حساب جديد').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=ليس لديك حساب؟').first).to_be_visible(timeout=30000)
