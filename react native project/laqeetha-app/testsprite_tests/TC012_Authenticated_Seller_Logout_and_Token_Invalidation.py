...
async def run_test():
    ...
    try:
        ...
        # Click on the 'بائع' (Seller) tab to switch to seller login form
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div/div/div/div/div/div/div[3]/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on the 'تسجيل دخول بائع' button to submit seller login form.
        frame = context.pages[-1]
        # Click on the 'تسجيل دخول بائع' (Seller Login) button to log in as seller
